...
/*  glutInit(&argc, argv);
  ...
  char title[58];
  sprintf(title, "CS296 Base Code for group 21. Running on Box2D %d.%d.%d", b2_version.major, b2_version.minor, b2_version.revision);
  ...
  //std:: cout << sttime << "  " << endtime << "  " << (endtime - sttime) << 	std::endl;
